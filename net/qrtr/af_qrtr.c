...
static void qrtr_port_remove(struct qrtr_sock *ipc)
{
	...
	/* Drop the port's reference only after the port is no longer
	 * reachable through qrtr_ports, so that a concurrent lookup can't
	 * find a socket whose refcount is about to hit zero.
	 */
	__sock_put(&ipc->sk);
}
